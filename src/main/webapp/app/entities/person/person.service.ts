...
export class PersonService {
  ...
  protected convertDateFromClient(person: IPerson): IPerson {
    const copy: IPerson = Object.assign({}, person, {
      lastActiveAt: person.lastActiveAt && person.lastActiveAt.isValid() ? person.lastActiveAt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.lastActiveAt = res.body.lastActiveAt ? moment(res.body.lastActiveAt) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((person: IPerson) => {
        person.lastActiveAt = person.lastActiveAt ? moment(person.lastActiveAt) : undefined;
      });
    }
    return res;
  }
